...
nx.draw(
    g, pos, with_labels=club_labels is not None,
    labels=club_labels,
    node_color=colors,
    ax=ax, **kwargs)
...
#############################################################
######## Compute D_tilde^{-1/2} A_tilde D_tilde^{-1/2} ######
#############################################################
A = nx.to_numpy_array(g)
A_mod = A + np.eye(g.number_of_nodes())     #A_tilde = A + I
...
anim.save('embed_anim.gif', dpi=300)
